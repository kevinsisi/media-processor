...
def test_custom_roi_overrides_smart_camera(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        video_renderer.auto_reframe,
        "build_filter_chain",
        lambda *args, **kwargs: "CUSTOM_ROI_CHAIN",
    )
    monkeypatch.setattr(
        video_renderer,
        "_smart_camera_filter",
    ...
    _paths, reframed = video_renderer.cut_segments(
        ...
        intermediate_dir=tmp_path / "out",
        target_aspect="9:16",
        tracking_target_by_asset={1: -1},
        custom_roi_by_asset={1: {"frames": [{"t_ms": 0, "x": 10, "y": 10, "w": 20, "h": 20}]}},
        smart_camera_enabled=True,
    )
...
def test_explicit_tracking_uses_lower_jitter_steady_crop_candidate(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    src = tmp_path / "asset.mp4"
    src.write_bytes(b"fake")
    ...
    steady_path = video_renderer.auto_reframe.CropPath(
        crop_w=960,
        crop_h=540,
        src_w=1920,
        src_h=1080,
