...
def test_derive_returns_none_kind_when_mid_band_area() -> None:
    """A single cluster with mean_area between 0.25 and 0.60 → kind=none."""
...
def test_no_move_directives_cover_every_cut() -> None:
    """When Vision is unavailable, every cut records an intentional no-move."""
    plan = CutPlan(
...
def test_serialise_round_trip_none_kind() -> None:
    ...
    assert rebuilt.from_rect == (0.0, 0.0, 1.0, 1.0)
...
def test_deserialise_rejects_malformed_blob() -> None:
    ...
    assert scp.deserialise_directive({}).kind == "none"  # type: ignore[union-attr]
    assert scp.deserialise_directive({"kind": "wibble"}).kind == "none"  # type: ignore[union-attr]
