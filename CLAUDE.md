...
- Production was rolled back to `0.30.22` after the `0.30.23` through `0.30.38` camera-motion experiments produced unacceptable output on draft `49`; `0.40.0` moves stabilization to an optional asset-level raw/stabilized variant workflow so analysis, tracking, and render use one selected source version.
- Camera-motion work must preserve this priority order: explicit user tracking intent, creative camera movement, smoothed tracking crop path, high-frequency stabilization cleanup, final output crop.
- Smart Camera `none` means "do not add an AI move". It must not force a static crop, vidstab fallback, or any extra correction.
- Point/custom/object tracking is a composition target, not raw per-frame crop control. Smooth it with dead zones, speed limits, acceleration limits, and drift guards before it reaches ffmpeg.
- Digital stabilization is a cleanup layer only. It may reduce high-frequency shake but must not choose framing, override user tracking, override Smart Camera movement, or create single-frame jumps.
- Do not judge camera-motion fixes only by whole-cut aggregate p95. Check adjacent-frame step spikes, localized problem windows, worker logs showing which path ran, and the rendered MP4.
...
- `ROADMAP.md` — Phase 6–10 全程路線圖（已完成/規劃中），含每個 sub-task 驗收標準。新對話開頭先讀這個就能對齊大方向。**目前版本：0.41.1。**
...
- The code itself — render pipeline:
  ...
  - `services/asset_variants.py` (v0.40.0) is the source-of-truth for raw/stabilized asset source selection. Use `selected_media_path(asset)` for any source read in analysis, point/custom tracking, Smart Camera/render input gathering, and future media probes. `Asset.file_path` remains immutable raw; `stabilized_path` is optional derivative. Switching variants clears coordinate-dependent tracking/analysis state before re-analysis so raw coordinates are never applied to stabilized video.
...
- The code itself — schema + API surface:
  ...
  - `models/asset.py` — `tracking_json` (v0.16) / `tracked_object_index` + `custom_roi_json` (v0.17, alembic 0012; v0.30.10 API surfaces `custom_roi_origin` from `custom_roi_json.init` so the FE can display the saved box) / `subtitle_secondary_*` (v0.19, alembic 0017) / `point_tracking_json` + `point_tracking_origin` (v0.23.0, alembic 0021); `tracked_object_index = -4` is the sentinel for "use point_tracking_json". v0.40.0 adds `stabilized_path`, `stabilization_status`, `stabilization_error`, and `active_asset_variant` (alembic 0028); existing assets default to raw/not_started.
  - `api/routers/assets.py` — v0.40.0 adds `POST /assets/{id}/stabilize` for source-level vidstab derivative generation and `PATCH /assets/{id}/variant` for raw/stabilized selection. Stabilized selection requires `done` + existing file; switching variants clears coordinate-dependent analysis/tracking and re-enqueues analysis by default. Enqueue failures must become terminal `failed` states when polling would otherwise hang.
  - `api/routers/uploads.py` — video upload completion still enqueues analysis, and v0.40.0 best-effort enqueues asset stabilization while leaving raw active. If stabilization enqueue fails after marking pending, persist `stabilization_status="failed"` and `stabilization_error="enqueue failed: ..."` so the operator can retry instead of watching a stuck pending state.
  ...
  - `services/project_fork.py` (v0.41.0) — `POST /projects/{id}/fork` creates an independent project copy for safe experiments. It copies project settings, script, assets, raw files, done stabilized derivatives, tags, segments, transcripts, coverage, BGM/watermark, and thumbnails into fork-owned ids/paths. It intentionally does NOT copy Draft rows, rendered outputs, reviews, comments, subtitle cues, or exports; the fork's first render starts at version 1. Missing required source media returns 409 after DB rollback + best-effort cleanup of already-copied files.
  - `api/routers/projects.py` (v0.41.1) — `POST /projects/{id}/assets/stabilize` batch-enqueues source-level stabilization for all project assets still at `not_started` / `failed` (default) while skipping `pending` / `running` / `done`. Single-asset enqueue failures are recorded per row as terminal `failed` and do not abort the rest of the batch.
