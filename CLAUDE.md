...
- The code itself —
  - `services/edit_planner.py` per-asset Gemini fanout (M6) + emotion / motion / face fields on `_AssetScore` + `_assemble_plan` 3-pass dedup/top-up (M8.1) + `StylePresetParams` + 5-preset bundles (v0.19 — span / transition / BGM hint per preset; `Draft.style_preset` column + alembic 0016)
  - `services/video_renderer.py` xfade chain (v0.19 re-introduces fade / dissolve / fadeblack / fadewhite for slow / artistic / commercial presets) + drawtext subtitle burn-in + `_zoompan_filter` (d=1, gated on motion-OR-face) + auto-reframe sendcmd chain (v0.16) + per-asset tracking-target dispatch (v0.17) + `apply_watermark` 9-grid PNG overlay (v0.18)
