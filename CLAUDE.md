...
- `ROADMAP.md` — Phase 6–10 全程路線圖（已完成/規劃中），含每個 sub-task 驗收標準。新對話開頭先讀這個就能對齊大方向。**目前版本：0.42.5。**
...
- The code itself — render pipeline:
  ...
  - `services/video_renderer.py` xfade chain (v0.19 re-introduces fade / dissolve / fadeblack / fadewhite for slow / artistic / commercial presets; transitions=None gates plain-mux concat) + drawtext subtitle burn-in (`SubtitleStyle` dataclass v0.19; secondary track v0.19.4) + `_zoompan_filter` (d=1, gated on motion-OR-face) + auto-reframe sendcmd chain (v0.16) + per-asset tracking-target dispatch (v0.17) + `apply_watermark` 9-grid PNG overlay (v0.18). v0.23.4: `_cut_segment` returns bool indicating whether the dynamic `crop@reframe` chain was applied; `cut_segments` returns `(paths, reframed_flags)`; `stabilize_segments` takes `skip_indexes` so vidstab does NOT re-translate already-subject-stabilised segments. **v0.29.0**: `ASPECT_DIMENSIONS` narrowed to `9:16` (1080×1920) + `16:9` (1920×1080); `aspect_filter(target_aspect, crop_region=(x_norm, y_norm))` — centre or `None` skips the explicit `x:y`, off-centre emits a clamped `crop=W:H:max(0,min(in_w-W, x_norm*(in_w-W))):...` expression. Anchor only consulted on the static aspect-crop path; auto-reframe (YOLO / point / custom_roi) ignores it. **v0.42.5**: opt-in AI Smart Camera keeps explicit user tracking (point / custom_roi / picked YOLO object) above Smart Camera; automatic YOLO can still be overridden by Smart Camera. `_smart_camera_filter` accepts `kind="none"` as no extra movement and no render-time visible-boost constants remain — planner directive rects are the single source of truth.
  - `services/smart_camera_planner.py` (v0.30.0; schema v3 in v0.42.5) per-cut Gemini Vision call → `FocusRegion` list → directive derivation. Rule-set: chronological disjoint clusters → `pan`; simultaneous clusters → `kind="none"`; single cluster + `mean_area < 0.25` → `zoom_in`; single cluster + `mean_area > 0.60` → `zoom_out`; empty / mid-band / Vision failure → `kind="none"`. Ease=`exp` for energetic motion (`pan/tilt/handheld`), `linear` otherwise. Up to `MAX_FRAMES_PER_CUT=4` JPEGs sampled per cut via ffmpeg seek-then-decode; partial failures serialize no-move directives instead of deterministic fallback motion. Prompt skill: `skills/gemini-prompts/smart-camera-focus/SKILL.md`.
