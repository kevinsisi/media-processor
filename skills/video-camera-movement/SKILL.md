...
- Tracking-aware post-stabilization must be accepted per cut based on measured output jitter. Render multiple measured crop-path / post-stabilization presets when needed; evaluate crop sidecars after post-stabilization, because a crop path can be worse alone but best after a steady stabilizer pass. If no candidate is clearly better than the original tracking crop, keep the original tracking crop.
