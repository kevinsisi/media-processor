...
> 目前版本：**0.43.6**（M9.15.39 — key-manager service-to-service default avoids mixed-tailnet DNS）
...
| **M9.13** | **像素追蹤改為非同步 RQ job：worker-analysis 跑 LK 不限時，前端 polling** | ✅ done | **0.28.0** |
| **M9.13.1** | **匯出 artifact 持久化 + 狀態 UX 可靠性修正** | ✅ done | **0.28.1** |
| **M9.13.2** | **小白 IG / FB 短影音工作流：安全上傳、社群預設、發佈工作台** | ✅ done | **0.28.2** |
| **M9.13.3** | **背景任務生命週期強化：enqueue / watchdog / cancellation / stale worker guard** | ✅ done | **0.28.3** |
| **M9.13.4** | **小白文案清理：主流程避免工程/剪輯術語，狀態與失敗提示改成下一步導向** | ✅ done | **0.28.4** |
| **M9.14** | **比例縮減（9:16 + 16:9）+ 來源↔輸出方向不同時的靜態裁切錨點選擇器** | ✅ done | **0.29.0** |
...
| **M9.15.10** | **追蹤焦點「框選區域」回傳並顯示已儲存 ROI，框選時 YOLO 主角框不再攔截拖曳** | ✅ done | **0.30.10** |
| **M9.15.11** | **第一次產生成品前可設定原始影片聲音音量，支援直接靜音原聲並套用到新片段** | ✅ done | **0.30.11** |
| **M9.15.12** | **處理狀態視窗正確展開，素材卡優先顯示跟住主角 / 點選位置與字幕入口** | ✅ done | **0.30.12** |
| **M9.15.13** | **處理狀態視窗改用 portal 並恢復內容自適應高度，避免 header 內跑版與空白大黑盒** | ✅ done | **0.30.13** |
| **M9.15.14** | **UI/UX 收斂：共用確認對話框、社群輸出文案對齊 9:16/16:9、手機 header / tracking 觸控與上傳流程防呆** | ✅ done | **0.30.14** |
| **M9.15.15** | **自動 YOLO 動態裁切路徑平滑：修正手持錄影左右晃動，保留已選片段與手動追蹤鎖定** | ✅ done | **0.30.15** |
...
| **M9.15.34** | **Edit-time mode selection：開始剪輯時可選一般剪輯 / 高級車質感 / 短片特效型，draft-scoped 保存並影響 planner prompt** | ✅ done | **0.43.5** |
| **M9.15.35** | **Narrato Story mode：分析頁可產生 StoryScript，剪輯頁可選 Narrato 故事模式，沿用既有 draft / renderer / review / export 工作流** | ✅ done | **0.43.5** |
| **M9.15.36** | **Story TTS narration：StoryScript 可選產生旁白音訊，依實測音訊長度延展畫面，TTS 不可用時保留字幕-only fallback** | ✅ done | **0.43.5** |
| **M9.15.37** | **NarratoAI documentary/drama modes：新增素材幀分析、紀錄片解說與短劇解說模式，使用 StoryScript + story_tts artifacts 走既有 render/review/export** | ✅ done | **0.43.5** |
| **M9.15.38** | **NarratoAI integration hardening：API/前端接受 documentary/drama_explain，edge-tts 進 dependencies，OpenSpec delta 與 prompt skills 補齊** | ✅ done | **0.43.5** |
| **M9.15.39** | **Key-manager service-to-service default：設定頁 sync 預設改用 `http://100.126.226.79:7823`，避免 `key.sisihome.org:7823` 吃 mixed-tailnet DNS timeout** | ✅ done | **0.43.6** |
...
## ✅ Phase 9.12（M9.12）— 多 worker 平行（已完成 0.27.0）

主題：**讓 5 個 worker 同時跑**，把 AMD 3700X 的 16 個 thread 跟 RTX 2070 的 8GB VRAM 都壓出來。

詳細 OpenSpec：`openspec/changes/archive/2026-05-04-v0.27-multi-worker/`。

### 9.12.1 切 worker 為三種 service
- 原本：一個 `worker:` container 同時 listen `analysis / editing / bgm` 三條 queue，慢的 MusicGen run 會 head-of-line block 一個剛上傳完要 analyze 的素材。
- 現在：三個 service 各 listen 自己的 queue：
  - `worker-analysis` — 1 個 container，nvidia GPU，跑 Whisper / YOLO / MediaPipe / Gemini Vision。模型載入 + VRAM 是瓶頸不是 concurrency。
  - `worker-editing` — `--scale=3`，**沒有** GPU reservation，跑 ffmpeg cut / concat / vidstab / reframe / subtitles / watermark / mix。每個 ffmpeg 已經是多 thread，三個並行才能把剩下的 7 個 idle core 用滿。
  - `worker-bgm` — 1 個 container，nvidia GPU，跑 MusicGen-small。跟 analysis 共享 RTX 2070；analysis 是 upload-time、bgm 是 render-time，實務上很少撞期。
- 同一個 `docker/worker.Dockerfile` build 出三個 image — `command:` CLI arg 決定 listen 哪條 queue，`deploy.resources.devices` 決定能不能看到 GPU。editing worker 雖然 image 內含 torch / whisper / musicgen，但 editing job functions 沒 import 這些 module，CUDA libs 永遠 dormant。
- `docker compose up -d --scale worker-editing=3` 是**真的 load-bearing**。`deploy.replicas: 3` 在純 `docker compose`（非 swarm）會被忽略；`--scale` 是唯一會被 honour 的方式。

### 9.12.2 worker name 不再衝突
- 原本：`Worker(queues, name=f"media-worker-{settings.api_host}")`，`api_host` 預設 `0.0.0.0`，**所有** container 算出同一個 worker name，RQ 的 worker registry 只認得到一個。pre-0.27 單 container 看不出來，多 container 直接互相蓋掉。
- 現在：`Worker(queues, connection=...)` 不傳 `name`，RQ 自動用 `hostname.pid` 唯一字串（Docker 給每個 container 不同 hostname）。`workers/__main__.py` 同時加 `sys.argv[1:]` 解析 + `VALID_QUEUES` guard，CLI 跑 `python -m media_processor.workers <queue>` 會驗證 queue name；不傳 arg 會 fallback 到 listen 全部三條（保留 backward compat 跟 dev 用法）。

### 9.12.3 queue inspector 改成 list[running]
- `QueueStatusOut.running` 從 `QueueJobItem | None` 改成 `list[QueueJobItem]`（**breaking schema**）。FE `QueueStatusBadge` 跟 `QueueStatusModal` 同步更新：
  - Badge：`處理中 N +M`（N 個跑中、M 個排隊）/ `排隊 M`，Pulse animation 在 `runningCount > 0` 時顯示。
  - Modal：「目前處理中（N）」section 改 `<ul>`，每個 row 跟排隊清單一樣支援 `highlightDraftId` 標「你的任務」。
- `queue_status` 端點原本只回每條 queue 的第一個 running job（`if running is None: running = item`），現在直接 collect `StartedJobRegistry.get_job_ids()` 全部塞進 list。pre-0.27 deploy 仍會 work — single worker 永遠只有一個 running，回出來就是 `[item]` 或 `[]`。

### 9.12.4 硬體實際配置
- AMD 3700X（8C/16T）：api / postgres / redis / worker-analysis / worker-bgm baseline 約占 9 thread；剩下 7 thread 給 3 個 worker-editing 平均 ≈ 2.3 thread/worker，加上 ffmpeg 自己的多 thread 不會 oversubscribe。
- 48GB RAM：分析路徑同時跑會吃到 6–8GB（Whisper medium + YOLO + MediaPipe + transcripts buffer），剪輯路徑每個 ffmpeg ~1GB → 三個 ≈ 3GB，加 BGM 路徑 ~5GB（MusicGen + 30s WAV buffer），合計 ≈ 17GB peak，留 30GB 給 page cache + postgres shared buffers。
- RTX 2070 8GB：Whisper medium ≈ 1.5GB、MusicGen-small fp32 ≈ 4GB、YOLO ≈ 0.5GB。analysis 跟 bgm 串接時最多 6GB，過得了。實際撞期罕見，因為 analysis 是 upload 觸發、bgm 是 render 觸發。

---

## ✅ Phase 9.12.1（M9.12.1）— 強制刪素材一步到位（已完成 0.27.1）

主題：**素材清理不要再走兩步**。v0.26 的 active-draft 409 block 在「我就是要清掉這個素材」的場景下太煩 — 操作員得先在 ProjectEdit 拒絕每個用到它的 draft，再回 ProjectAnalysis 刪。bulk cleanup 動輒 4–5 次跨頁切換。

詳細 OpenSpec：`openspec/changes/archive/2026-05-04-v0.27.1-asset-force-delete/`。

### 9.12.1.1 後端：`AssetDeleteResult` 取代 `AssetInUseError`
- `services/asset_management.delete_asset(session, id, *, force=False)` 從 raise-on-block 改成 return-an-outcome：
  - `not_found=True` → endpoint 翻 404（保留原語意）
  - `deleted=False, affected_drafts=[BlockingDraft(draft_id, version, status), ...]` → asset 沒動，等 FE 確認
  - `deleted=True` → 該砍的都砍掉了；`invalidated_versions` 列出剛被翻成 failed 的 draft versions
- `force=True` 路徑：
  1. 對每個 active draft，`DELETE FROM draft_segments WHERE draft_id=? AND asset_id=?` — 直接幹掉指向這個 asset 的 segments，**不**動 parent draft row。`flush()` 後重數該 draft 還剩幾個 segment。
  2. 剩 0 個 → `draft.status = "failed"` + `draft.prompt_feedback = "素材已被刪除"`。**不**砍 draft row — 留著訊息給操作員看。
  3. 剩 >0 個 → draft 留著（半空狀態，下次 render 會自爆，但這是 force-delete 的代價，不是我們要在這裡解的問題）。
  4. 接續 v0.26 的 cascade-delete failed/rejected drafts：那個 query 是 `JOIN DraftSegment ON asset_id=?`，剛被翻成 failed 的 drafts segments 已經被我們刪掉了，不會被 join 到 — 所以剛剛寫進 `prompt_feedback` 的訊息**不會**被連帶幹掉。
  5. 然後正常 disk + DB 砍 asset。
- `batch_delete_assets` 多吃一個 `force` kwarg，per-row 把 `AssetDeleteResult` 整個吐出來；endpoint 翻成 `AssetBatchDeleteResultItem` 加 `affected_drafts` + `invalidated_versions` 兩個欄位。

### 9.12.1.2 後端：endpoint 不再 409
- `DELETE /assets/{id}?force=true` 從 204 No Content + 409 Conflict 改成永遠 200 + `AssetDeleteOut` body。`deleted=False` + 非空 `affected_drafts` 是 FE 「該確認 force 了」的訊號；`deleted=True` + `invalidated_versions` 是「這些 draft 剛被標為失敗」的回報。
- `DELETE /projects/{id}/assets/batch?force=true` 結構平行：response body 多 `needs_force_count` + `error_count` 兩個欄位拆開「要二次確認」跟「真的失敗」兩種 blocked。

### 9.12.1.3 前端：一個 confirm dialog 列完所有受影響的 draft
- `ProjectAnalysis.runBatchDelete`：先發一次 `force=false` 的批次刪除。如果 response 的 `needs_force_count > 0`，組一個 confirm dialog：
  ```
  3 個素材正被使用中：
  素材 #39 被 v1 使用中
  素材 #41 被 v2、v3 使用中
  ...
  ```
  使用者按確定 → 用同一個 ids 列表發第二次 `force=true`。整個流程兩次 API call、一次使用者輸入。
- 結果 panel 加 `invalidated_versions` 顯示：「素材 #39：連帶將 v1 標為失敗」。錯誤跟「真的有問題的 row」區分開（`affected_drafts === []` 但有 `reason` 才是真錯誤）。
- API client：`deleteAsset(id, {force})` + `batchDeleteAssets(projectId, ids, {force})`。

### 9.12.1.4 取消 hard-block 的代價
- 操作員可能在沒看清楚的狀況下確認 force，把正在預覽的 draft 弄壞。對策是 confirm dialog 強制顯示每個受影響的 version 列表 — 滑過去也躲不掉。
- 半空狀態（draft 還有 segments 但不是這個 asset）的 draft 沒被自動標為 failed，下次 render 會自爆。這是知情選擇：操作員按了 force 就接受這個結果。

---

## ✅ Phase 9.13（M9.13）— 像素追蹤非同步化（已完成 0.28.0）

主題：**精準像素追蹤要能跑完，不接受 fallback**。v0.27.3 的 30 s wall-clock budget 對 1728×3072 portrait 的 2 分鐘片是不夠的；v0.28.0 把 LK 整段搬到 worker-analysis 上跑，前端 polling 取代等待。

詳細 OpenSpec：`openspec/changes/archive/2026-05-05-v0.28-async-point-tracking/`。

### 9.13.1 alembic 0024：Asset 加兩欄
- `point_tracking_status: str | None`：NULL（pre-0.28 / 從未跑過）/ `"pending"` / `"done"` / `"failed"`。
- `point_tracking_error: str | None`：worker 失敗時的訊息，給 FE toast 用。
- 兩欄 nullable；renderer 不讀 status — 只看 `point_tracking_json is not None`，所以 pre-0.28 既有的 trace 仍然 render 得出來（向後相容）。

### 9.13.2 新 RQ job：`track_point_job` on `analysis` queue
- `workers/point_tracking_jobs.py:track_point_job(asset_id, init_norm_x, init_norm_y, init_t_ms)` — RQ entry，跟 `analyze_asset` 一樣 wrap `asyncio.run` 呼叫 async 編排器。
- `services/point_tracking_runner.py:run_point_tracking` — async 編排：load Asset → 跑 `track_point`（沒設 budget；以 RQ 的 30 min `default_timeout` + runner 內 1 h time_budget_s 為防呆）→ 寫回 `point_tracking_json` + `point_tracking_origin` + `status="done"`。任何 exception 都被捕捉並寫 `status="failed"` + `error`，這樣 FE polling 不會永遠看到 pending。
- 排隊 inspector 也加入新 kind `point_track`，FE 顯示「精準像素追蹤」。

### 9.13.3 endpoint 改為 enqueue + 202
- `PATCH /assets/{id}/tracking-target` mode=point：先把 `tracked_object_index = -4`、`status = "pending"`、`point_tracking_json = NULL`、`point_tracking_origin = {frame_ms, norm_x, norm_y}`（先不寫 x/y，等 worker 用 cv2 解析）寫進 row；commit；enqueue；回 **202 Accepted** + 完整 `TrackingTargetResponse`。其他 mode（auto/object/custom/fixed/none）保持同步 + 200。
- 移除原本 endpoint 內的 `asyncio.to_thread(track_point)` + 30 s budget + 504 處理。

### 9.13.4 前端 polling
- `AssetTrackingTarget.tsx`：PATCH 收到 202 後 `applyTarget` 立刻 setBusy(false)；新的 `useEffect` 看 `detail.point_tracking_status === "pending"` 就每 2 s 跑一次 `fetchDetail`，flip 為 `"done"` 或 `"failed"` 就停。
- `done` 時 crosshair 自動繪製（`point_tracking_origin.x/y` 由 worker 寫進來）；`failed` 時把 `point_tracking_error` 抄進 `error` state，在錯誤面板顯示「追蹤失敗：{reason}」。
- 卡片底部新增一行：「精準像素追蹤分析中…（worker 正在跑 LK 光流，較長 / 高解析度的素材需要幾分鐘）」— pending 期間就算操作員關掉 picker 也會看到狀態。

### 9.13.5 不變的東西
- LK 演算法本身（`track_point` in `services/point_tracking.py`）跟 v0.23 起的 root-cause 修正全部保留 — v0.23.7 的 cv2 post-rotation 解析、v0.23.5 的 sendcmd 雙指令合併、v0.23.4 的 vidstab/reframe 互斥都還在。只是執行位置從 api 搬到 worker。
- `point_tracking_json` 的 schema 完全沒變 — `auto_reframe.compute_crop_path_from_point_track` 不需動。
- v0.27.3 的 `MAX_LK_DURATION_S = 30` 仍然是 `track_point` 的 default budget；只有 worker runner 顯式 override 為 1 h。沒有 caller 直接帶 `time_budget_s` 進來時行為不變。

### 9.13.6 v0.28.1：匯出與狀態 UX 可靠性
- 新 `draft_exports` table（alembic 0025）：每次 `POST /drafts/{id}/export` 會先建立 artifact row，worker 再更新 `queued → running → done/failed`，讓重新整理後仍能看到匯出狀態。
- `GET /drafts/{id}/exports` 回傳該 draft 的 derivative export 清單；完成時提供 `/api/media/drafts/{project_id}/{filename}` 下載 URL。
- Export sheet 不再顯示不存在的「專案下載清單」文案，改為直接列出已建立匯出與下載按鈕。
- `useDraftPolling` / `useAssetPolling` 加 generation + in-flight guard，避免慢 response 覆蓋較新的 route/state。
- Header queue badge API 失敗時顯示「狀態未知」，不再把錯誤偽裝成 `排隊 0`。
- `/projects/:id/edit` 的 analysis gate 改用固定 6 步驟檢查；缺 step / 空 payload 會視為尚未 ready，不會提前開放剪輯。
- P1 UX audit 已寫入 `openspec/changes/improve-export-and-status-ux/`：技術詞替換、confirm 改 sheet、ProjectEdit 主路徑收斂、Analysis 手機 bottom bar、legacy Review redirect。

### 9.13.7 v0.28.2：小白 IG / FB 短影音工作流
- `Upload.tsx` 的下一步改為先保存目前腳本文字再導到素材分析；若保存失敗，留在上傳頁並顯示錯誤，避免快速點下一步時丟最後 edits。
- `ExportSheet` 改以 Instagram/Facebook 目的地預設為主：IG/FB Reels、IG/FB 直式貼文、方形貼文，全部沿用現有 `{aspect, height}` export artifact API。Reels 使用同一張卡，避免 9:16 1080p 重複匯出任務。
- 原始比例 / 清晰度控制保留在「進階」區，讓小白先看平台目的地，熟手仍能自訂 9:16 / 4:5 / 1:1 與 720p / 1080p / 1440p。
- `/projects/:id/edit` ready state 新增「發佈工作台」：預覽、下載主成品、建立 IG/FB 版本、重新產生一版成為第一層操作；片段順序、字幕、配樂、品牌標誌微調收成次要進階區。
- `/projects/:id/review` 不再顯示 legacy placeholder workflow，直接 redirect 到 `/projects/:id/edit`。
- 主路徑文案移除最顯眼的 `Gemini` / `FFmpeg` / `vidstab` / `YOLO` / `worker` / `渲染` 說法，改成「產生成品」「素材檢查」「自動跟住主角」「畫面防手震」等小白能理解的結果語言。

### 9.13.8 v0.28.3：背景任務生命週期強化
- `POST /projects/{id}/edit`、draft re-render/order/rebuild、export、BGM、point tracking、analysis、translation enqueue 失敗時不再留下沒有 RQ job 的 pending/running row；會 rollback 到 terminal response 或寫入 failed 狀態讓 UI 停止 polling。
- `api.watchdog` 不只看 Draft render orphan，現在也 reconcile `draft_exports` queued/running、`bgm_generation_jobs` pending/running、`Asset.point_tracking_status="pending"`、`Asset.status="analyzing"`；Redis 掃描錯誤維持 fail-open，不會因 Redis 短暫不通誤殺任務。
- `services.queue` 新增通用 RQ queued + StartedJobRegistry 掃描 helper，可用 queue/function/args/kwargs/job_id 比對 durable intent。
- Worker guard：stale render job 不能 adopt 非 pending draft；late stage/ready/failed update 不覆蓋 terminal draft；export worker 會檢查 artifact 的 draft/aspect/height intent；BGM/point tracking late success 不覆蓋 terminal state。
- Generic queued-job cancel 會同步 durable state：render/export/BGM/point tracking/analysis 會進 terminal failed/cancelled-like state；running render 仍走 domain cancel endpoint。

### 9.13.9 v0.28.4：小白文案完整收斂
- 主流程術語固定為「素材檢查 / 產生成品 / 建立下載版本 / 品牌標誌」；`queue` / `worker` / `RQ` / `render` / `export` 等工程詞只留在程式碼註解、API 名稱或開發者錯誤細節。
- 上傳、素材檢查、成品頁、發佈工作台、匯出 sheet、處理狀態 modal、配樂、主角跟拍、字幕、品牌標誌與設定頁文案統一改成下一步導向。
- 進階控制仍保留比例、清晰度、時間軸、轉場、配樂與字幕等熟手能力，但 copy 先描述使用者結果，再補必要細節。

---

## ✅ Phase 9.15（M9.15）— AI Smart Camera / AI 智慧運鏡（0.30.0，已實作）

主題：**讓靜態 cut 也有運鏡語言**。Subject-tracking（M9.1–9.2）+ point-tracking（M9.8）已涵蓋追拍主角的需求；但無追蹤目標、emotion=neutral 的 cut 目前還是平鋪直敘。M9.15 在 plan generation 階段對每個 cut 多打一隻 Gemini Vision，拿到視覺重點區域（focus_regions）後推導三種運鏡 directive（zoom_in / zoom_out / pan），並依音樂節拍對齊 ease。

實作位置：`services/smart_camera_planner.py`（rule-set + Gemini Vision 呼叫 + 序列化）+ `services/video_renderer._smart_camera_filter`（zoompan-driven crop）+ `services/edit_orchestrator` 新增 stage（plan persist 之後、render 之前；partial-success）。Render 端走 zoompan 而非 time-varying crop（後者 ffmpeg 不支援可變 output 尺寸）。

OpenSpec：`openspec/changes/ai-smart-camera/proposal.md` + `tasks.md`。

### 9.15.0 預設關閉是 load-bearing
- `Project.smart_camera_enabled` default `False`，nullable=False，alembic 0027 加欄位（SQLite 走 batch mode，Postgres 直接 add）。
- `EditTriggerRequest.smart_camera: bool | None`，沿用 v0.21.1 的 render-flags-persistence：snapshot 進 `Draft.render_flags_json["smart_camera"]`，priority body > snapshot > project toggle > false。
- 理由 1（Gemini quota）：每 cut 多燒 N tokens（N≈4–6 frames + prompt），若預設打開會默默吃 free tier。
- 理由 2（操作員偏好）：M8.1 emotion zoompan 上線時就有「我要靜態鏡頭」的客訴，這次 opt-in 起跳。

### 9.15.1 三種策略 + 一個 fallback
- `mean_area < 0.25` 且 1 disjoint cluster → `zoom_in`（1.0×→1.4× 推近 focus 中心）
- `mean_area > 0.60` 且 1 cluster → `zoom_out`（1.3×→1.0× 從 focus 拉開）
- `≥ 2 disjoint clusters`（IoU < 0.10） → `pan`（from_rect = cluster[0]，to_rect = cluster[-1]，ease=linear）
...
- v0.23.5 的「同 timestamp 必須 x/y 合併成一條 directive」修正延伸到這裡
- **不要**自己寫第二份 sendcmd writer

### 9.15.5 Skill 抽出
- `skills/gemini-prompts/smart-camera-focus/SKILL.md` 作為 canonical reference（仿 asset-score / scene-tag）
- `CLAUDE.md` 的 Skill Activation Rules 加一條
