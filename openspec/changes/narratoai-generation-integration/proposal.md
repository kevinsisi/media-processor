...
- **擴充 `story_tts` 服務**：以既有 Story/Narrato TTS artifact pipeline 為基礎，支援 edge-tts provider，將解說文案逐條合成音訊並以實測 duration 對齊 timeline。
...
- **新增 Asset 欄位**：`frame_analysis_json` / `frame_analysis_status` / `frame_analysis_error`。TTS provider/voice 走既有 `story_tts_*` runtime settings；解說音訊走 `story_narration_assets` artifact，不新增 Draft narration 欄位。
...
- `tts-synthesis`: StoryScript 解說文案 → story_tts artifact + measured duration
...
- **新 Python 依賴**：`edge-tts`（TTS 引擎）
- **新 alembic migration**：Asset 新增 `frame_analysis_json` / `frame_analysis_status` / `frame_analysis_error`
- **新 worker 工作量**：幀分析（Vision LLM fan-out，worker-analysis 佇列）+ TTS 合成（CPU，worker-editing 佇列）
- **新 API 端點**：`POST /assets/{id}/frame-analysis` / `GET /assets/{id}/frame-analysis`。TTS 由 StoryScript-backed render flow 直接觸發，不提供獨立 draft synthesis endpoint。
