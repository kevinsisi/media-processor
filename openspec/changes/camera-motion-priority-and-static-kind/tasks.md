...
- [x] T4 — Priority flip: explicit tracking wins
  - [x] `services/video_renderer.py` `_cut_segment`:
    ...
    - [x] `reframed_flags` value for the cut: `True` iff the segment was
  ...
  - [x] `tests/unit/test_video_renderer.py`:
    ...
    - [x] Add `test_kind_none_does_not_set_reframed_flag` asserting
...
- [x] T7 — CI / local validation
  - [x] `make typecheck` (`mypy src`) clean.
  - [x] `make lint` (`ruff check src tests`) clean.
  - [x] `make test` (`pytest -v`) all green; smart-camera planner +
    video renderer test files MUST run.
  - [x] `TRACKING_FAKE=1 pytest tests/unit/test_video_renderer.py`
