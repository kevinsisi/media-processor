...
- **0.23.6 — bbox 中心 rounding**：`compute_crop_path_from_point_track` 之前用 `(int(x-0.5), int(y-0.5), w=1, h=1)` 合成 1×1 bbox，期望 `cx = x + w//2` 落在 LK 像素。但 `int(x-0.5)` 在浮點 LK output 上 floor 成 floor(x-0.5)，後面 `+ 1//2 = 0` 沒補正，centre 比真實 LK 像素左 1 px。改成 `(int(round(x)), int(round(y)), w=0, h=0)`，centre = round(x)。
- **0.23.7 — rotation-aware norm→pixel resolution**：v0.23.6 之後大部分素材都對齊了，但有一個「車頭片段」一直系統性偏左。追蹤每一步座標發現 asset 18（DJI 4K 直拍）有 rotation metadata：ffprobe 讀的 stream 維度 = 3840×2160 landscape（pre-rotation），但縮圖跟 OpenCV 4.13 默認 `CAP_PROP_ORIENTATION_AUTO=1` 看到的都是 2160×3840 portrait（post-rotation）。API 之前 `init_x = round(norm_x × Asset.resolution.W)` = `round(0.48 × 3840) = 1848`，clamp 到 cv2 2160 寬框架後落在 86%（不是用戶想的 48%）。修法：重構 `track_point` 改吃 `init_norm_x` / `init_norm_y`，內部用 `cv2.CAP_PROP_FRAME_WIDTH/HEIGHT`（旋轉後維度）換算像素，整個 pipeline 對 source dimension 只有「cv2 看到什麼」這個唯一真理。Migration：旋轉素材的舊 point_tracking_json 必須在 UI 重新 pick 一次。
